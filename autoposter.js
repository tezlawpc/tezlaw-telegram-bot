...
//  FIX v2: Runs on startup + correct Pacific timezone scheduling
// ============================================================
...
function loadState() {
  ...
  return { lastImmigrationCheck: null, lastWeatherCheck: null, publishedTitles: [], weeklyEvergreen: { pi: null, business: null, trademark: null, estate: null }, lastHolidayPost: null };
}
...
async function notifyTeam(message) {
  ...
  try {
    await axios.post(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, { chat_id: TEAM_CHAT_ID, text: message, parse_mode: "Markdown" });
  ...
  try {
    ...
    if (res.data.length > 0) {
      ...
      for (const post of res.data) {
        ...
        if (matches.length >= 4) { console.log(`⚠️ WP duplicate found: "${post.title.rendered}"`); return true; }
      }
    }
...
function isEvergreenTopicUsedRecently(topicKey, state) {
  ...
  if (daysSince < 28) { console.log(`⚠️ Evergreen topic "${topicKey}" used ${Math.round(daysSince)} days ago — skipping`); return true; }
  return false;
}
...
async function translatePost(post, language) {
  const langConfig = {
    chinese: { label: "Chinese (Traditional)", instruction: "Translate this entire WordPress blog post to Traditional Chinese (繁體中文). Keep all HTML tags exactly as they are. Only translate visible text. For the author box, keep 章律師 as-is. Keep URLs, phone numbers, and email addresses unchanged.", categoryPrefix: "中文-", tagSuffix: " 中文", titleSuffix: " | 章律師移民法律事務所" },
    spanish: { label: "Spanish", instruction: "Translate this entire WordPress blog post to Spanish (Latin American Spanish). Keep all HTML tags exactly as they are. Only translate visible text. Keep URLs, phone numbers, and email addresses unchanged.", categoryPrefix: "Español-", tagSuffix: " español", titleSuffix: " | Tez Law P.C." }
  };
  ...
  const prompt = `${cfg.instruction}\n\nORIGINAL TITLE: ${post.title}\n\nORIGINAL ARTICLE BODY (HTML):\n${articleBody}\n\nReturn ONLY a JSON object (no backticks):\n{"title":"translated title","content":"translated HTML body","metaDescription":"translated meta (150-160 chars)","focusKeyword":"primary keyword in ${cfg.label}"}`;
  try {
    ...
    return { title: translated.title, content: fullContent, category: cfg.categoryPrefix + post.category, tags: (post.tags || []).map(t => t + cfg.tagSuffix), metaDescription: translated.metaDescription, focusKeyword: translated.focusKeyword };
...
async function publishAllLanguages(post, notifyPrefix) {
  ...
  if (wpDup) { console.log("⚠️ Skipping — WordPress duplicate detected for:", post.title); return 0; }
  ...
  if (chinesePost) {
    try { const published = await publishToWordPress(chinesePost); results.push({ lang: "中文", link: published.link }); console.log("✅ Published Chinese:", chinesePost.title); }
    catch (e) { console.error("Chinese publish failed:", e.message); }
  }

  const spanishPost = await translatePost(post, "spanish");
  if (spanishPost) {
    try { const published = await publishToWordPress(spanishPost); results.push({ lang: "Español", link: published.link }); console.log("✅ Published Spanish:", spanishPost.title); }
...
async function generatePost({ topic, practiceArea, context, useSearch }) {
  const locationContext = practiceArea === "Immigration Law" || practiceArea === "Immigration"
    ...
    : "Southern California, specifically Los Angeles County, Orange County, San Bernardino County, and Riverside County. Key cities include West Covina, Los Angeles, Anaheim, San Bernardino, Riverside, Ontario, Pomona.";
...
REQUIREMENTS:
1. TITLE (under 65 characters, include primary keyword + location or year)
...
3. CONTENT (1,000-1,400 words):
   ...
   - H2: Frequently Asked Questions
     * 3 FAQ items as <div class="faq-item"><h3>Question?</h3><p>Answer</p></div>
...
{"title":"SEO title","metaDescription":"150-160 char meta","content":"full HTML content","category":"Immigration|Personal Injury|Business Law|Trademarks|Estate Planning","tags":["tag1","tag2","tag3","tag4","tag5"],"focusKeyword":"main SEO keyword"}`;
  ...
  try {
    ...
    if (firstParaMatch) {
      const humanizePrompt = `${JJ_VOICE}\n\nRewrite ONLY these two opening paragraphs in JJ Zhang's voice. Return ONLY the rewritten HTML paragraphs:\n\n${firstParaMatch[1]}`;
      ...
      if (humanizedIntro && humanizedIntro.includes("<p>")) {
        ...
        console.log("✅ Post intro humanized");
      }
    }
  } catch (e) { console.log("Humanization failed, using original:", e.message); }
...
async function checkImmigrationNews(state) {
  ...
  const prompt = `Search for the most significant US immigration law news or development from the past 7 days. Pick the single most newsworthy item relevant to US immigration (visas, green cards, deportation, work permits, asylum).

Respond in this exact JSON format only:
{"hasNews":true,"headline":"brief headline here","summary":"one sentence summary here"}
...
If nothing noteworthy: {"hasNews":false}`;
  ...
  if (isDuplicateTitle(newsData.headline, state)) { console.log("⚠️ Duplicate detected:", newsData.headline); return 0; }

  await new Promise(r => setTimeout(r, 10000));
  let post = null;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      post = await generatePost({ topic: newsData.headline, practiceArea: "Immigration Law", context: newsData.summary, useSearch: false });
      if (post) break;
    } catch (e) { console.log(`Post generation attempt ${attempt} failed:`, e.message); if (attempt < 3) await new Promise(r => setTimeout(r, 15000)); }
  }
  if (!post) return 0;
  ...
  try {
    ...
    return count > 0 ? 1 : 0;
  } catch (e) { console.error("Failed to publish:", e.message); return 0; }
}
...
async function checkWeather(state) {
  ...
  try {
    ...
    const periods = forecastRes.data.properties.periods.slice(0, 3);
    ...
    if (!isRainy) { console.log("No rain forecast, skipping."); return 0; }
    const now = Date.now();
    if (state.lastWeatherCheck && (now - state.lastWeatherCheck) < 3 * 24 * 60 * 60 * 1000) { console.log("Weather post recently published."); return 0; }
    const rainType = /thunderstorm|flood/i.test(weatherText) ? "severe storm" : "rain";
    const post = await generatePost({ topic: `Car Accidents During ${rainType === "severe storm" ? "Storms" : "Rain"} in California`, practiceArea: "Personal Injury", context: `Weather: ${weatherText.substring(0, 200)}`, useSearch: false });
    if (!post) return 0;
    const count = await publishAllLanguages(post, "🌧️ *Weather Post Published!*");
...
async function checkHolidays(state) {
  ...
  for (const holiday of HOLIDAYS) {
    ...
    if (daysUntil >= 0 && daysUntil <= 2) {
      ...
      console.log(`🎉 Holiday approaching: ${holiday.name}`);
      const post = await generatePost({ topic: holiday.topic, practiceArea: "Personal Injury", context: `${holiday.name} in ${daysUntil} day(s).`, useSearch: false });
      if (!post) continue;
      const count = await publishAllLanguages(post, "🎉 *Holiday Post Published!*");
      if (count > 0) { state.lastHolidayPost = holidayKey; state.publishedTitles.push(post.title); return 1; }
    }
  }
  return 0;
}
...
const EVERGREEN_TOPICS = {
  ...
  estate: ["Why Every California Homeowner Needs a Living Trust","How Probate Works in California — And How to Avoid It","Prop 19 and California Property Tax — What Families Need to Know in 2026","What Happens If You Die Without a Will in California?"],
};
...
async function checkEvergreen(state) {
  ...
  if (dayOfWeek === 2 && state.weeklyEvergreen.pi !== now.toDateString()) {
    ...
    if (post) {
      const count = await publishAllLanguages(post, "📝 *PI Post Published!*");
      if (count > 0) { state.weeklyEvergreen.pi = now.toDateString(); state.publishedTitles.push(post.title); postsPublished++; }
    }
  }
  ...
  if (dayOfWeek === 4 && state.weeklyEvergreen.business !== now.toDateString()) {
    ...
    if (weekNum % 2 === 0) {
      ...
      if (post) { const count = await publishAllLanguages(post, "📝 *Business Law Post Published!*"); if (count > 0) { state.weeklyEvergreen.business = now.toDateString(); state.publishedTitles.push(post.title); postsPublished++; } }
    }
  }
  ...
  if (dayOfWeek === 4 && state.weeklyEvergreen.trademark !== now.toDateString()) {
    ...
    if (weekNum % 2 === 1) {
      ...
      if (post) { const count = await publishAllLanguages(post, "📝 *Trademark Post Published!*"); if (count > 0) { state.weeklyEvergreen.trademark = now.toDateString(); state.publishedTitles.push(post.title); postsPublished++; } }
    }
  }
  ...
  if (dayOfWeek === 5 && state.weeklyEvergreen.estate !== now.toDateString()) {
    ...
    if (weekNum % 2 === 0) {
      ...
      if (post) { const count = await publishAllLanguages(post, "📝 *Estate Planning Post Published!*"); if (count > 0) { state.weeklyEvergreen.estate = now.toDateString(); state.publishedTitles.push(post.title); postsPublished++; } }
    }
  }
...
async function runDailyScheduler() {
  ...
  try { totalPublished += await checkImmigrationNews(state); } catch (e) { console.error("Immigration check error:", e.message); }
  try { totalPublished += await checkWeather(state); } catch (e) { console.error("Weather check error:", e.message); }
  try { totalPublished += await checkHolidays(state); } catch (e) { console.error("Holiday check error:", e.message); }
  try { totalPublished += await checkEvergreen(state); } catch (e) { console.error("Evergreen check error:", e.message); }

  saveState(state);
  console.log(`✅ Auto-poster complete. Published ${totalPublished} post(s) today.\n`);
}

// ── FIXED SCHEDULER: runs on startup + 8 AM Pacific daily ────
function scheduleDaily() {
  // ✅ FIX 1: Run immediately on startup to catch missed posts
  console.log("🚀 Running auto-poster on startup (catching any missed posts)...");
  setTimeout(async () => {
    await runDailyScheduler();
  }, 10000); // 10-second delay to let server fully initialize

  // ✅ FIX 2: Use UTC 15:00 = 8 AM Pacific (PDT) for daily schedule
  // Render servers run UTC. Pacific Daylight Time = UTC-7, so 8 AM PDT = 15:00 UTC
  function scheduleNext() {
    const now = new Date();
    const next = new Date();
    next.setUTCHours(15, 0, 0, 0); // 8 AM PDT
    if (next <= now) next.setUTCDate(next.getUTCDate() + 1);

    const msUntil = next - now;
    const hoursUntil = Math.round(msUntil / 1000 / 60 / 60 * 10) / 10;
    console.log(`⏰ Next scheduled auto-post in ${hoursUntil} hours (8 AM Pacific / 15:00 UTC)`);
