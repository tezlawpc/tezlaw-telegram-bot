...
const SYSTEM_PROMPT = `
Your name is Zara. You are a const express = require("express");
const axios = require("axios");
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    if (message.voice || message.audio) {
      ...
      const duration = (message.voice || message.audio).duration || 0;

      // Reject files over 10 minutes
      if (duration > 600) {
        await sendMessage(chatId, "That voice message is too long. Please keep it under 10 minutes or type your question.");
        return;
      }
      ...
      try {
        ...
        if (!transcript) {
          await sendMessage(chatId, "Sorry, I couldn't make out that voice message. Could you type your question instead?");
          return;
        }

        // Echo transcript so client knows what was heard
        await sendMessage(chatId, `🎤 I heard: "${transcript}"\n\nLet me help with that...`);

        // Process transcription through Zara with memory
        const reply = await askClaudeWithMemory("telegram", String(chatId), transcript, SYSTEM_PROMPT);
        await sendMessage(chatId, reply);

        const urgency = detectDistress(transcript);
        if (urgency !== "none") {
          await notifyTeamDistress(chatId, transcript, urgency, "Telegram Voice");
        }
      } catch (err) {
        console.error("Voice processing error:", err.message);
        await sendMessage(chatId, "I had trouble with that voice message. Please try again or type your question.");
      }
      return;
    }
    ...
    if (userText === "/reset") {
      await clearHistory("telegram", String(chatId));
    ...
    const reply = await askClaudeWithMemory("telegram", String(chatId), userText, SYSTEM_PROMPT);
    await sendMessage(chatId, reply);
...
warm, friendly legal assistant for Tez Law P.C. in West Covina, California.
